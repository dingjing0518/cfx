			...
			formatter : function(value, row, index) {
				...
				if (row.orderStatus > 0) {
					str += ' <a class="save" href="javascript:;" style="text-decoration:none;">'
							+ '<button type="button" style="display:none;" showname="FC_ODER_MGR_BTN_DOWNLOAND" class="btn btn-warning" onclick="javascript:downPDF(\''
							+ row.orderNumber
							+ '\');">下载合同</button></a>';
				}
            
				str += ' <a class="save" href="javascript:;" style="text-decoration:none;">'
						+ '<button type="button" data-toggle="modal" data-target="#orderGoods" style="display:none;" showname="FC_ODER_MGR_BTN_ORDERGOODS" class="btn btn-warning" onclick="javascript:orderGoods(\''
						+ row.orderNumber
						+ '\',  \''  + row.block+ '\',  \''  + row.economicsGoodsType+ '\');">订单商品</button></a>';
                
                str += ' <a class="save" href="javascript:;" style="text-decoration:none;">'
						+ '<button type="button" data-toggle="modal" data-target="#payMentVoucherId" style="display:none;" showname="FC_ODER_MGR_BTN_VOUCHER" class="btn btn-warning" onclick="javascript:orderPaymentVoucher(\''
						+ row.orderNumber
						+ '\');">上传付款凭证</button></a>';
                
         	   if(row.orderStatus==3||row.orderStatus==5){
                str += ' <a class="save" href="javascript:;" style="text-decoration:none;">'
					+ '<button type="button" data-toggle="modal" data-target="#payMentVoucherId" style="display:none;" showname="FC_ODER_MGR_BTN_BILLOFLOADING" class="btn btn-warning" onclick="javascript:orderDelivery(\''
					+ row.orderNumber
					+ '\');">订单提货</button></a>';
                }
...
function downPDF(orderNumber) {
	var confirm = new $.flavr({
		...
		dialog : 'confirm',
		onConfirm : function() {
			window.location.href = getRootPath() + "/uploadPDF_F.do?orderNumber="
					+ orderNumber;
			confirm.close();
		}
	});
 ...
 * @param orderNumber
 */
function orderGoods(orderNumber,block,economicsGoodsType) {
	var cfg = {
...
function orderDelivery(orderNumber){
