...
var columns = [	{
	...
	formatter : function(value, row, index) {
	   var str="";
	   str+='<button type="button" style="display:none;" showname="MARKET_CONTRACT_MANAGE_BTN_DOWNLOAND" class="btn btn-warning" onclick="javascript:downContractPDF(\''+ row.contractNo+ '\',\'' + row.block + '\');">下载合同</button></a>';
	   str+='<button type="button" style="display:none;" showname="MARKET_CONTRACT_MANAGE_BTN_CONTRACTGOODS" class="btn btn-warning" onclick="javascript:contractGoods(\''+ row.contractNo+ '\',\'' + row.block + '\');">合同商品</button></a>';
        if(row.contractStatus != -1) {
        	str += '<button type="button" style="display:none;" showname="MARKET_CONTRACT_MANAGE_BTN_CLOSE" class="btn btn-warning" onclick="javascript:contractCLOSE(\'' + row.contractNo + '\');">合同关闭</button></a>';
        }
	   return str ;
	}
}, {
...
function byOrderStatus(contractStatus){
    ...
    var cfg = {
        url : './searchMContractList.do'+urlParams(1),
        columns : columns,
        uniqueId : 'pk',
        sortName : 'insertTime',
        paginationShowPageGo: true,
...
function contractCLOSE(pk){
    var confirm = new $.flavr({
        ...
        onConfirm : function() {
            ...
            $.ajax({
                type : 'POST',
                url : './updateContract.do?contractNo='+pk,
...
function exportContact(){
    ...
    $.ajax({
        type : 'POST',
        url : './exportMContact.do'+urlParams(1),
        data : {},
        dataType : 'json',
        success : function(data) {
            if (data.success){
