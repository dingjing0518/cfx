			...
			formatter : function(value, row, index) {
				...
				if (row.orderStatus > 0) {
					str += ' <a class="save" href="javascript:;" style="text-decoration:none;">'
							+ '<button type="button" style="display:none;" showname="OPER_ORDER_BTN_DOWNLOAND" class="btn btn-warning" onclick="javascript:downPDF(\''
							+ row.orderNumber
							+ '\');">下载合同</button></a>';
				}
                if (row.orderStatus != -1) {
                    str += ' <a class="save" href="javascript:;" style="text-decoration:none;">'
                        + '<button type="button" data-toggle="modal"  style="display:none;" showname="OPER_ORDER_BTN_CLOSE" class="btn btn-warning" onclick="javascript:closeOrder(\''
                        + row.orderNumber
                        + '\');">关闭订单</button></a>';
                }
				str += ' <a class="save" href="javascript:;" style="text-decoration:none;">'
						+ '<button type="button" data-toggle="modal" data-target="#orderGoods" style="display:none;" showname="OPER_ORDER_BTN_ORDERGOODS" class="btn btn-warning" onclick="javascript:orderGoods(\''
						+ row.orderNumber
						+ '\',  \''  + row.block+ '\');">订单商品</button></a>';
                if (row.orderStatus == -1){
				str += ' <a class="save" href="javascript:;" style="text-decoration:none;">'
                    + '<button type="button" data-toggle="modal" data-target="#creditCustomerTrack" style="display:none;" showname="OPER_ORDER_BTN_ORDERTRACK" class="btn btn-warning" onclick="javascript:contractTrack(\''
                    + row.orderNumber+ '\');">订单追踪</button></a>';
                }
                str += ' <a class="save" href="javascript:;" style="text-decoration:none;">'
						+ '<button type="button" data-toggle="modal" data-target="#payMentVoucherId" style="display:none;" showname="OPER_ORDER_BTN_VOUCHER" class="btn btn-warning" onclick="javascript:orderPaymentVoucher(\''
						+ row.orderNumber
						+ '\');">上传付款凭证</button></a>';
				return str;
			}
