...
public class DimensionalityExtPresentRunnable  implements Runnable {
	private String name;
	...
	public DimensionalityExtPresentRunnable() {

	}

	
	public DimensionalityExtPresentRunnable(String name ,String uuid) {
	...
	private void upLoadFile() {
		...
		if (storeDao != null) {
			...
			map.put("methodName", "exportDimensionalityExtPresent_"+StringUtils.defaultIfEmpty(this.uuid, ""));
			// 查询存在要导出的任务
			...
			if (list != null && list.size() > Constants.ZERO) {
				for (SysExcelStoreExtDto storeDto : list) {
					...
					this.fileName = "运营中心-会员体系-额外奖励赠送-" + storeDto.getAccountName() + "-"
						...
						if (b2bDimensionalityPresentExDao != null) {
							String ossPath = "";
							 ...
							 parmsMap.put("pks", params.getIds().split(","));
								...
								List<B2bDimensionalityPresentExDto>  presentExDtos =  b2bDimensionalityPresentExDao.searchGridExtWard(parmsMap);
								 if (presentExDtos != null && presentExDtos.size() > Constants.ZERO) {
										...
										String path = exportUtil.exportDynamicUtil("dimensionalityPresentExtGive", this.fileName, presentExDtos);
	...
	private void setColAwardParams(Map<String, Object> parmsMap, String accountPk) {
			if (!cn.cf.common.utils.CommonUtil.isExistAuthName(accountPk, ColAuthConstants.OPER_MEMBER_DIMEMSEXT_COL_COMPANYNAME)){
				parmsMap.put("companyNameCol",ColAuthConstants.OPER_MEMBER_DIMEMSEXT_COL_COMPANYNAME);
			}
			if (!cn.cf.common.utils.CommonUtil.isExistAuthName(accountPk, ColAuthConstants.OPER_MEMBER_DIMEMSEXT_COL_MOBILE)){
				parmsMap.put("contactTelCol",ColAuthConstants.OPER_MEMBER_DIMEMSEXT_COL_MOBILE);
			}
	}
	...
	public static String dimensionalityExtPresentParam(CustomerDataTypeParams params, SysExcelStore store) {
		String paramStr = "";
