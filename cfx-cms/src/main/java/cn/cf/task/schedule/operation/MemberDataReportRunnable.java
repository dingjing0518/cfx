...
public class MemberDataReportRunnable implements Runnable {
	...
	public MemberDataReportRunnable() {

	}

	public MemberDataReportRunnable(String name,String uuid) {
	...
	private void upLoadFile() {
		...
		if (storeDao != null) {
			...
			map.put("methodName", "exportMemberData_"+StringUtils.defaultIfEmpty(this.uuid, ""));
			// 查询存在要导出的任务
			...
			if (list != null && list.size() > Constants.ZERO) {
				for (SysExcelStoreExtDto storeDto : list) {
					...
					this.fileName = "运营中心-数据报表-会员数据-" + storeDto.getAccountName() + "-"
	...
	private String setNotLimitPages(Query query, MongoTemplate mongoTemplate, SysExcelStoreExtDto storeDto) {
		String ossPath = "";
		List<MemberDataReport> list = mongoTemplate.find(query, MemberDataReport.class);
		if (list != null && list.size() > Constants.ZERO) {
			ExportUtil<MemberDataReport> exportUtil = new ExportUtil<MemberDataReport>();
			String path = exportUtil.exportDynamicUtil("memberData", this.fileName, list);
	...
	private String setLimitPages(Query query, int counts, MongoTemplate mongoTemplate, SysExcelStoreExtDto storeDto) {
		...
		for (int i = Constants.ZERO; i < numbers + Constants.ONE; i++) { //
			...
			if (list != null && list.size() > Constants.ZERO) {
				ExportUtil<MemberDataReport> exportUtil = new ExportUtil<MemberDataReport>();
				String excelName = "运营中心-数据报表-会员数据-" + storeDto.getAccountName() + "-"
						+  DateUtil.formatYearMonthDayHMS(new Date())+"-"+(i+1);
				String path = exportUtil.exportDynamicUtil("memberData", excelName, list);
				File file = new File(path);
				fileList.add(file);
			}
		}
	...
	public static String memberDataReportParam(ReportFormsDataTypeParams params, SysExcelStore store) {
		String paramStr = "";
