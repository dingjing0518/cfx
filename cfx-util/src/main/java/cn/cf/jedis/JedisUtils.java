...
public class JedisUtils {
	...
	// Redis的端口号
	private static int PORT = PropertyConfig.getIntProperty("REDIS_PORT");
		...
		try {
			...
			if (jedis.exists(key)) {
				...
				if (null != str && !"".equals(str)) {
					String demoArray[] = str.replace("\"", "").split(",");
					list = new ArrayList<String>();
					for (int i = 0; i < demoArray.length; i++) {
						list.add(demoArray[i]);
					}
				}
			}
		} catch (Exception e) {
	...
	public static boolean setList(String key, List<String> list, int seconds) {
		...
		try {
			...
			String objectJson = JsonUtils.convertToString(sb.toString());
			jedis = getJedis();
			jedis.set(key, objectJson);
			if (seconds != 0) {
				jedis.expire(key, seconds);
	...
	public static void lock(String goodsPk) {
		...
		while (lockFlag) {// 循环等待拿锁
			lockFlag = JedisUtils.acquireLock(goodsPk);
			if (num == 0) {
				JedisUtils.expire(goodsPk, 60);
			}
			num++;
		}
