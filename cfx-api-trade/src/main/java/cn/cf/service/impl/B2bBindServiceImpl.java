...
public class B2bBindServiceImpl implements B2bBindService {
    ...
    public PageModelBind<B2bBindDtoEx> searchBindList(Map<String, Object> map) {
        ...
        for (int i = 0; i < b2bBindDtoList.size(); i++) {
        	...
        	if (null!=bindGoodsList && bindGoodsList.size()>0){
				...
				for (int j = 0; j < bindGoodsList.size(); j++) {
					leftCount=leftCount+(null==bindGoodsList.get(j).getBoxes()?0:bindGoodsList.get(j).getBoxes());
					totalCount=totalCount+(null==bindGoodsList.get(j).getTotalBoxes()?0:bindGoodsList.get(j).getTotalBoxes());
				}
				if (totalCount!=0) {
        ...
        //页签数量
        //pm.setCountMap(b2bBindDaoEx.getBindListCount(map));
        //上架状态，0：全部，1：上架，2：下架
