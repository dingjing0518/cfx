...
public class LogisticsErpList {
	...
	public LogisticsErpList(Map<String, String> map, B2bMemberDto dto) {
		...
		// 1业务员+省份+城市+区县+乡镇+厂区
		if (dto.getEmployeeNumber() == null|| "".equals(dto.getEmployeeNumber())) {
			map.put("memberPk",dto.getPk() == null ? "" : dto.getPk());
		}else{
		...
		map1 = new HashMap<String, String>();
		map1.putAll(map);
		if(map1.containsKey("employeeNumber")){
			map1.remove("employeeNumber");
		}else{
			map1.remove("memberPk");
		}
